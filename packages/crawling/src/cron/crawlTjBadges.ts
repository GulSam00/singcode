...
// 실패한 곡은 badges가 null로 남아 다음 청크 조회에 다시 딸려 나온다.
// 그대로 두면 실패가 쌓일수록 매 청크가 같은 곡을 재시도하고,
// 실패 수가 청크 크기를 넘으면 진행이 멈춘 채 무한 반복된다. 이번 실행 동안은 건너뛴다.
...
while (true) {
  ...
  const songs = (await getSongsBadgeNullDB(requested)) as TargetSong[];
  if (songs.length === 0) break;

  // 이번 실행에서 이미 실패한 곡은 제외한다. 남은 게 전부 실패 이력이면 더 진행할 게 없다.
  const targets = songs.filter(song => !failedIds.has(song.id));
  if (targets.length === 0) {
    console.log(`⏭️  남은 ${songs.length}곡은 모두 이번 실행에서 실패한 곡이라 종료합니다.`);
    break;
  }
  ...
  if (DRY_RUN) {
    console.log(`🧪 [DRY RUN] ${targets.length}곡 조회, 저장 대상 ${results.length}건`);
    console.table(
  ...
  console.log(
    `📦 진행 ${processed}곡 / 저장 ${updated}건 / 실패 ${failedIds.size}건 (${elapsed}s)`,
  );
